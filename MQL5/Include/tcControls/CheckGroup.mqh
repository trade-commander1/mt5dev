...
public:
   ...
   { 
        if(idx >= 0 && idx <= ArraySize(m_rows))
            return m_rows[idx].Color();
        return clrBlack;
   }
   bool              Color(int idx,const color value)  
   { 
        int sz=ArraySize(m_rows);
        if(idx >= 0 && idx < sz)
            return m_rows[idx].Color(value);
        // for all
        if(idx < 0)
        {
            bool bret=true;
            for(int i=0;i < sz;++i)
                bret &= m_rows[i].Color(value);
            return bret;
        }
        return false;
   }

   color             ColorBackground(int idx)         const 
   { 
        return  m_background.ColorBackground();     
   }
   bool              ColorBackground(const color value)  
   { 
      bool bret=true;
      m_background.ColorBackground(value);        
      for(int i=0;i < ArraySize(m_rows);++i)     
         bret &= m_rows[i].ColorBackground(value);
   ...
   { 
        return m_background.ColorBorder();      
   }
   bool              ColorBorder(const color value)  
   { 
         bool bret=true;
         m_background.ColorBorder(value);      
         for(int i=0;i < ArraySize(m_rows);++i)     
            bret &= m_rows[i].ColorBorder(value);
         return bret;
   }   

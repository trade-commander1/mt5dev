...
public:
   // Color Frame and Titlebar
   void ColorFrameAndTitlebar(const color clrBk,const color clrFg);
protected:
...
void CDialog::ColorFrameAndTitlebar(const color clr,const color clrFg)
{
   ...
   for(int i=0;i<total;i++)
   {
          ...
          //if(StringFind(name,prefix+"Border")!=-1)
           {
             ...
             {
                // restore
                edit.Color(CONTROLS_DIALOG_COLOR_CAPTION_TEXT);
         ...
         else if(StringFind(name,prefix+"Border")!=-1)
         {
             CPanel *ppanel=(CPanel*) obj;
             if(clr != clrNONE)
             {
                ppanel.ColorBackground(clr);
                ppanel.ColorBorder(clr);
             }
             else
             {
                // restore
                ppanel.ColorBackground(CONTROLS_DIALOG_COLOR_BG);
                ppanel.ColorBorder(CONTROLS_DIALOG_COLOR_BG);
             }
...
// tc
protected:
   string createFieldName(string prefix="")
   {
      ++_FieldCounter;
      if(StringLen(prefix) > 0)
         return StringFormat("%s_%s_%d",m_name,prefix,_FieldCounter);
      else
         return StringFormat("%s_%d",m_name,_FieldCounter);
   }

   virtual bool              createClient(CWndObj& xc,int x1,int y1,int x2,int y2)
   {
      if(!xc.Create(m_chart_id,createFieldName(),m_subwin,x1,y1,x2,y2))
         return(false);
      if(!Add(xc))
         return(false);
      return true;           
   }
   virtual bool              createClient(CWndObj& xc,string text,int x1,int y1,int x2,int y2)
   {
      if(!xc.Create(m_chart_id,createFieldName(),m_subwin,x1,y1,x2,y2))
         return(false);
      if(!xc.Text(text))         
   ...
   virtual bool              createClient(CWndClient& xc,int x1,int y1,int x2,int y2)
   {
      if(!xc.Create(m_chart_id,createFieldName(),m_subwin,x1,y1,x2,y2))
         return(false);
      if(!Add(xc))
   ...
   virtual bool              createClient(CWndContainer& xc,int x1,int y1,int x2,int y2)
   {
      if(!xc.Create(m_chart_id,createFieldName(),m_subwin,x1,y1,x2,y2))
         return(false);
      if(!Add(xc))
   ...
   virtual bool              createClient(tcBarControl& xc,int x1,int y1,int x2,int y2,bool bborder=false)
   {
      if(!xc.Create(m_chart_id,createFieldName(),m_subwin,x1,y1,x2,y2,bborder))
         return(false);
      if(!Add(xc.edBar()))
         return(false);
      if(!Add(xc.lbText()))
   ...
   if(id==CHARTEVENT_CHART_CHANGE)
     {
      ...
      if(m_chart.HeightInPixels(m_subwin)<Height()+CONTROLS_BORDER_WIDTH)
        {
        // ce! auskommentiert. Macht nur Probleme

...
public:
   ...
   // TC
   color             ColorBackground(void)         const { return(m_label.ColorBackground());        }
   bool              ColorBackground(const color value)  { return(m_label.ColorBackground(value));   }
   color             ColorBorder(void)         const { return(m_label.ColorBorder());        }
   bool              ColorBorder(const color value)  { return(m_label.ColorBorder(value));   }      
   bool              SetColors(const color fg,const color bk=clrNONE,const color border=clrNONE) 
   {
      bool bret=true;
      if(fg != clrNONE)
      {
         bret &= m_label.Color(fg);   
      }
      if(bk != clrNONE)
      {
         bret &= m_label.ColorBackground(bk);   
      }
      if(border != clrNONE)
      {
         bret &= m_label.ColorBorder(border);   
      }
      return bret;
   }
   bool SetFont(const string name,const int size)
   {
      return m_label.SetFont(name,size);
   }

   //--- state
   bool              Checked(void)       const { return(m_button.Pressed());     }
   bool              Checked(const bool flag)  { isChecked_=flag;return(m_button.Pressed(flag)); }
   inline bool       isChecked(void) const {return isChecked_;}
   //--- data
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CCheckBox::CCheckBox(void) : m_value(0),isChecked_(false)
  {
  }
//+------------------------------------------------------------------+
//| Destructor                                                       |

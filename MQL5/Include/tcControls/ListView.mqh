...
public:
   ...
   //--- fill
   virtual bool      AddItem(const string item,const long value=0,ENUM_ALIGN_MODE am=ALIGN_LEFT,bool redraw=true);
   //--- data
   virtual bool      ItemAdd(const string item,const long value=0,ENUM_ALIGN_MODE am=ALIGN_LEFT,bool redraw=true);
   virtual bool      ItemInsert(const int index,const string item,const long value=0);
   virtual bool      ItemUpdate(const int index,const string item,const long value=0,bool redraw=true);
...
//! ce
public:
   color             tColor(int idrow) const 
   { 
      if(idrow >= 0 && idrow < m_total_view)
         return m_rows[idrow].Color();
      return clrBlack;      
   }
   bool              Color(int idrow,const color value)
   {
      if(idrow >= 0 && idrow < m_total_view)
      {
         m_rows[idrow].Color(value);
         return true;
      }
      return false;
   }
   color             EdiColorBackground(int idrow) const 
   { 
      if(idrow >= 0 && idrow < m_total_view)
         return m_rows[idrow].ColorBackground();
      return clrBlack;
   }
   bool              EdiColorBackground(int idrow,const color value)
   {
      if(idrow >= 0 && idrow < m_total_view)
      {
         m_rows[idrow].ColorBackground(value);
         return true;
      }
      return false;
   }

   ENUM_ALIGN_MODE   TextAlign(int idrow)        const 
   { 
      if(idrow >= 0 && idrow < m_total_view)
         return m_rows[idrow].TextAlign();  
      return ALIGN_LEFT;                 
   }
   bool              TextAlign(int idrow,const ENUM_ALIGN_MODE align)
   {
      if(idrow >= 0 && idrow < m_total_view)
      {
         m_rows[idrow].TextAlign(align);
         return true;
      }
      return false;
   }
...
//| Add item (row)                                                   |
//+------------------------------------------------------------------+
bool CListView::AddItem(const string item,const long value,ENUM_ALIGN_MODE am,bool redraw)
  {
//--- method left for compatibility with previous version
   return(ItemAdd(item,value,am,redraw));
  }
//+------------------------------------------------------------------+
//| Add item (row)                                                   |
//+------------------------------------------------------------------+
bool CListView::ItemAdd(const string item,const long value,ENUM_ALIGN_MODE am,bool redraw)
  {
//--- add
...
//| Update item (row)                                                |
//+------------------------------------------------------------------+
bool CListView::ItemUpdate(const int index,const string item,const long value,bool redraw)
  {
//--- update

...
bool              CWndObj::SetColors(const color fg,const color bk,const color border)
{
   bool bret=true;
   if(fg != clrNONE)
   {
   ...
   if(bk != clrNONE)
   {
      m_color_background=bk;
      bret &= OnSetColorBackground();   
   }
   if(border != clrNONE)
   {
      m_color_border=border;
      bret &= OnSetColorBorder();   
   }
   return bret;
}
...
//+------------------------------------------------------------------+  
bool              CWndObj::SetFont(const string value,const int size)
{
   m_font=value;
   m_font_size=size;
   return(OnSetFont() && OnSetFontSize());
}  
//+------------------------------------------------------------------+
